...
- `crawler-job`은 component scan 범위를 배치 실행에 필요한 `config`, `job`, `runner`, `infrastructure.jpa.adapter.crawler`로 제한한다.

## Code Boundaries

- `domain`: JPA 엔티티와 도메인 enum을 담는다.
- `application`: 화면/API use case, 추천 정책, 노트북 상세, 댓글, web-facing out port를 담는다.
- `application-crawler`: crawler 저장/동기화 use case, crawler 전용 out port, profile/score 정책, crawler command 계약을 담는다.
- `infrastructure-jpa`: Spring Data shared/web repository, Flyway migration, web-facing JPA application port adapter를 담는다.
...
- `infrastructure-jpa.adapter.shared/web/crawler`: 런타임 역할별 JPA adapter를 나눠 web context와 crawler context가 서로의 adapter를 스캔하지 않게 한다.
...
추천/상세/댓글의 사용자 흐름은 `application` use case가 직접 소유한다. 크롤러 저장, 프로필 동기화, 가격 이력, 추천 점수 갱신 흐름과 그 out port는 `application-crawler`에 둔다. 웹 추천 점수 계산처럼 사용자 요청에 필요한 보조 정책만 `application.service`에 둔다. crawler job 구현은 `job.crawler` 패키지에 둬 런타임 역할을 패키지명에서도 드러낸다.
`crawler-job`은 domain entity, `application.port.out`, `application.service` 내부 구현에 직접 의존하지 않고, `application-crawler`의 command/use case 계약만 호출한다. 추천/상세/댓글 같은 사용자 흐름 use case와 웹 추천 계산 서비스는 crawler runtime에 직접 등록하지 않는다.
