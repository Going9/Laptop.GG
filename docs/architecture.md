...
- `infrastructure-jpa-core`: shared Spring Data repository, Flyway migration, 공통 PostgreSQL/Flyway/JPA 설정을 담는다.
...
- `infrastructure-jpa-core.repository.shared`, `infrastructure-jpa.repository.web`, `infrastructure-jpa-crawler.repository.crawler`: 런타임 역할별 Spring Data repository를 나눠 web/crawler가 필요한 repository만 등록한다.
...
`crawler-job`은 shared repository와 Flyway 설정을 위해 `infrastructure-jpa-core`만 사용하고, web-facing `infrastructure-jpa` 모듈은 런타임 classpath에 올리지 않는다.
