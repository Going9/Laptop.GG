...
- `persistence-model`: JPA 엔티티와 DB 매핑 모델을 담는다.
- `persistence-model`의 JPA 엔티티는 Kotlin `data class`를 쓰지 않고, to-one 연관관계는 `fetch = LAZY`를 명시해 영속성 객체 정체성과 저사양 운영 조회 비용을 안정적으로 유지한다.
...
- `infrastructure-jpa-core`: Flyway migration, 공통 PostgreSQL/Flyway/JPA 설정을 담는다.
...
`crawler-job`은 `infrastructure-jpa-crawler`의 crawler 전용 adapter config facade만 직접 알고, Flyway와 공통 persistence 설정은 adapter 모듈 내부 의존으로 가져온다. web-facing `infrastructure-jpa` 모듈은 런타임 classpath에 올리지 않는다.
...
- Schema changes are managed by Flyway through the web deploy path. `crawler-job` does not run Flyway migrations against production; it validates the mapped schema through JPA and fails before crawling when the schema is not ready.
