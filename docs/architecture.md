...
`crawler-job`은 `infrastructure-jpa-crawler`의 crawler 전용 JPA adapter/config만 직접 알고, Flyway와 공통 persistence 설정은 adapter 모듈 내부 의존으로 가져온다. web-facing `infrastructure-jpa` 모듈은 런타임 classpath에 올리지 않는다.
