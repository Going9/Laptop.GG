...
추천/상세/댓글의 사용자 흐름은 use case가 직접 소유하고, 프로필/점수/가격 이력 같은 보조 정책만 `application.service`에 둔다. crawler job 구현은 `job.crawler` 패키지에 둬 런타임 역할을 패키지명에서도 드러낸다.
`crawler-job`은 domain entity나 `application.service` 내부 구현에 직접 의존하지 않고, application의 crawler command/use case 계약만 호출한다.
