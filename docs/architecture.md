...
- crawler 저장, 프로필 동기화, 가격 이력, 추천 점수 갱신 bean은 `application-crawler` 모듈에만 존재하며 web runtime classpath에 들어가지 않는다.
- `crawler-job`은 component scan 범위를 배치 실행에 필요한 `job`, `runner`, `application.crawler`, `infrastructure.jpa.adapter.shared/crawler`로 제한한다.

## Code Boundaries

- `domain`: JPA 엔티티와 도메인 enum을 담는다.
- `application`: 화면/API use case, 추천 정책, 노트북 상세, 댓글, application service와 out port를 담는다.
- `application-crawler`: crawler 저장/동기화 use case, profile/score 정책, crawler command 계약을 담는다.
- `infrastructure-jpa`: Spring Data repository, Flyway migration, JPA application port adapter를 담는다.
- `infrastructure-jpa-crawler`: crawler 저장 이력, 가격 이력, 추천 점수 JPA adapter와 repository를 담아 web runtime classpath에서 crawler persistence 구현을 제외한다.
- `infrastructure-jpa.adapter.shared/web/crawler`: 런타임 역할별 JPA adapter를 나눠 web context가 crawler 저장 adapter를 스캔하지 않게 한다.
- `infrastructure-jpa.repository.shared/web/crawler`: 런타임 역할별 Spring Data repository를 나눠 web/crawler가 필요한 repository만 등록한다.
- `infrastructure-jpa/src/main/resources/laptopgg-persistence.yml`: web/crawler가 공통으로 import하는 PostgreSQL, Flyway, JPA 운영 설정의 단일 출처다.
...
추천/상세/댓글의 사용자 흐름은 `application` use case가 직접 소유한다. 크롤러 저장, 프로필 동기화, 가격 이력, 추천 점수 갱신 흐름은 `application-crawler`에 두고, 웹 추천 점수 계산처럼 사용자 요청에 필요한 보조 정책만 `application.service`에 둔다. crawler job 구현은 `job.crawler` 패키지에 둬 런타임 역할을 패키지명에서도 드러낸다.
`crawler-job`은 domain entity나 `application.service` 내부 구현에 직접 의존하지 않고, `application-crawler`의 command/use case 계약만 호출한다. 추천/상세/댓글 같은 사용자 흐름 use case와 웹 추천 계산 서비스는 crawler runtime에 로딩하지 않는다.
`application` 테스트도 `infrastructure-jpa`를 의존하지 않는다. JPA repository나 Spring Data가 필요한 통합 테스트는 `infrastructure-jpa` 테스트로 둔다.
`crawler-job` 테스트는 수집 orchestration, parser, runner 경계만 검증한다. 저장 use case와 JPA adapter가 함께 필요한 persistence 통합 테스트는 `infrastructure-jpa` 테스트에 둔다.
