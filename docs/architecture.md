...
- Crawler workflow runs `ops/sql/crawler-identity-preflight.sql` against the production DB tunnel before executing the crawler jar. Duplicate `laptop.product_code` or `laptop.detail_page` groups must be diagnosed with `ops/sql/crawler-identity-diagnostics.sql` and cleaned before crawling.
