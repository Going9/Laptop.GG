...
- crawler 저장, 프로필 동기화, 가격 이력, 추천 점수 갱신 bean은 `crawler` 프로필에서만 로딩한다.
- `crawler-job`은 component scan 범위를 배치 실행에 필요한 `job`, `runner`, `application.crawler`, `infrastructure.jpa`로 제한한다.

## Code Boundaries

- `domain`: JPA 엔티티와 도메인 enum을 담는다.
- `application`: 화면/API use case, 추천 정책, 노트북 상세, 댓글, `application.crawler` 저장/동기화 use case, application service와 out port를 담는다.
...
추천/상세/댓글의 사용자 흐름은 use case가 직접 소유한다. 크롤러 저장, 프로필 동기화, 가격 이력, 추천 점수 갱신 흐름은 `application.crawler`에 두고, 웹 추천 점수 계산처럼 사용자 요청에 필요한 보조 정책만 `application.service`에 둔다. crawler job 구현은 `job.crawler` 패키지에 둬 런타임 역할을 패키지명에서도 드러낸다.
`crawler-job`은 domain entity나 `application.service` 내부 구현에 직접 의존하지 않고, application의 crawler command/use case 계약만 호출한다. 추천/상세/댓글 같은 사용자 흐름 use case와 웹 추천 계산 서비스는 crawler runtime에 로딩하지 않는다.
