package going9.laptopgg.job.crawler.danawa.client

import org.assertj.core.api.Assertions.assertThat
import org.junit.jupiter.api.Test

class DanawaRetryPolicyTest {
    private val retryPolicy = DanawaRetryPolicy(FixedDanawaJitterSource(jitterMillis = 17L))

    @Test
    fun `retries only retryable statuses before last attempt`() {
        assertThat(retryPolicy.shouldRetry(statusCode = 429, attempt = 0)).isTrue()
        assertThat(retryPolicy.shouldRetry(statusCode = 503, attempt = 1)).isTrue()
        assertThat(retryPolicy.shouldRetry(statusCode = 404, attempt = 0)).isFalse()
        assertThat(retryPolicy.shouldRetry(statusCode = 429, attempt = retryPolicy.maxAttempts - 1)).isFalse()
    }

    @Test
    fun `uses status-specific bounded retry delay`() {
        assertThat(retryPolicy.retryDelayMillis(attempt = 0, statusCode = 429)).isEqualTo(4_017L)
        assertThat(retryPolicy.retryDelayMillis(attempt = 0, statusCode = 403)).isEqualTo(2_517L)
        assertThat(retryPolicy.retryDelayMillis(attempt = 0, statusCode = 503)).isEqualTo(1_517L)
        assertThat(retryPolicy.retryDelayMillis(attempt = 10, statusCode = 429)).isEqualTo(8_017L)
    }

    private class FixedDanawaJitterSource(
        private val jitterMillis: Long,
    ) : DanawaJitterSource {
        override fun nextLong(maxInclusive: Long): Long {
            return jitterMillis.coerceAtMost(maxInclusive)
        }
    }
}
