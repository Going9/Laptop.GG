...
### 7-1. Oracle 앱 서버 nginx 설정
- Docker 기반 배포가 아니라 Oracle 인스턴스에서 `systemd + java -jar` 로 운영할 때는 [oracle-laptopgg.conf](/Users/iggyu/git/Laptop.GG/nginx/oracle-laptopgg.conf) 설정을 사용합니다.
- Docker compose 용 nginx 설정은 기존 [default.conf](/Users/iggyu/git/Laptop.GG/default.conf) 를 유지합니다.

적용 순서는 아래와 같습니다.
```bash
sudo cp /path/to/Laptop.GG/nginx/oracle-laptopgg.conf /etc/nginx/sites-available/laptopgg
...
### 7-2. 도메인과 HTTPS
- DNS 는 `laptopgg.com`, `www.laptopgg.com` 모두 앱 서버 공인 IP로 연결합니다.
- 최초 연결과 인증서 발급 시점에는 Cloudflare 프록시를 잠시 끄고 `DNS only` 로 두는 편이 가장 단순합니다.
- 인증서 발급 뒤 Cloudflare 프록시를 다시 켤 수 있으며, 이 경우 SSL/TLS 모드는 `Full (strict)` 를 권장합니다.
...
### 7-3. 크롤러 수동 실행 팁
- GitHub Actions `Crawl Laptops` 의 `테스트용 처리 개수 제한` 칸에는 `100` 처럼 숫자만 넣습니다.
- `limit=100` 형식으로 입력하면 실패합니다.
- 빈칸으로 두면 전체 크롤링이 실행됩니다.

## 8. 저장소 구조
- 현재 저장소 루트 자체가 Gradle 프로젝트 루트입니다.
- 예전처럼 `LaptopGG/` 하위 디렉터리로 한 번 더 들어갈 필요가 없습니다.
- 주요 파일 위치는 아래와 같습니다.
  - 애플리케이션 코드: [src](/Users/iggyu/git/Laptop.GG/src)
  - Gradle 설정: [build.gradle.kts](/Users/iggyu/git/Laptop.GG/build.gradle.kts), [settings.gradle.kts](/Users/iggyu/git/Laptop.GG/settings.gradle.kts)
  - Gradle wrapper: [gradlew](/Users/iggyu/git/Laptop.GG/gradlew)
  - 운영용 GitHub Actions: [ci.yml](/Users/iggyu/git/Laptop.GG/.github/workflows/ci.yml), [deploy-web.yml](/Users/iggyu/git/Laptop.GG/.github/workflows/deploy-web.yml), [crawler.yml](/Users/iggyu/git/Laptop.GG/.github/workflows/crawler.yml)
